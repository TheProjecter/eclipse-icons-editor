...
public class CrawlISharedImagesAction extends Action {
	...
	public void run() {
		...
		iconImageDescriptorText = new Text(shell, SWT.SINGLE | SWT.BORDER
				| SWT.READ_ONLY);
		...
		Listener paintListener = new Listener() {
			public void handleEvent(Event event) {
				...
				case SWT.PaintItem: {
					...
					int x = event.x;
					event.gc.drawImage(image, x,
							event.y + (itemHeight - image.getBounds().height)
									/ 2);
			...
			if (Modifier.isPublic(f.getModifiers())
				...
				if (f.getAnnotation(Deprecated.class) == null) {
					try {
						...
						String fieldValue = f.get(ISharedImages.class)
								.toString();
						// Check if deprecated
						if (!isDeprecatedInISharedImages(fieldValue)) {
							ImageDescriptor imageDescriptor = PlatformUI
		...
		shell.pack();
		shell.setBounds(shell.getBounds().x, shell.getBounds().y,
	...
	public static final String[] deprecatedList = { "IMG_OBJ_PROJECT",
			"IMG_OBJ_PROJECT_CLOSED", "IMG_OBJS_BKMRK_TSK",
			"IMG_OBJS_TASK_TSK", "IMG_OPEN_MARKER", "IMG_TOOL_BACK_HOVER",
			"IMG_TOOL_COPY_HOVER", "IMG_TOOL_CUT_HOVER",
			"IMG_TOOL_DELETE_HOVER", "IMG_TOOL_FORWARD_HOVER",
			"IMG_TOOL_NEW_WIZARD_HOVER", "IMG_TOOL_PASTE_HOVER",
